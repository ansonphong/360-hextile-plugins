...
# Usage: ./scripts/sync-hextile.sh [--push]
...
if [[ ! -d "$ROOT/.git" ]]; then
  echo "sync-hextile: not a git repo: $ROOT" >&2
  exit 2
fi
if [[ ! -f "$ROOT/.gitmodules" ]]; then
  echo "sync-hextile: missing .gitmodules in $ROOT" >&2
  exit 2
fi
if [[ ! -e "$ROOT/$SUB" ]]; then
  echo "sync-hextile: missing submodule path $SUB" >&2
  exit 2
fi
...
if [[ -n "$staged_any" ]]; then
  echo "sync-hextile: refuse — index already has staged paths:" >&2
  echo "$staged_any" >&2
...
if [[ -d "$ROOT/$SUB/.git" ]] || [[ -f "$ROOT/$SUB/.git" ]]; then
  if ! git -C "$ROOT/$SUB" diff --quiet 2>/dev/null \
    ...
    echo "sync-hextile: refuse — dirty working tree in $SUB" >&2
    git -C "$ROOT/$SUB" status -sb >&2 || true
    exit 1
  fi
  untracked="$(git -C "$ROOT/$SUB" ls-files --others --exclude-standard | head -1 || true)"
  if [[ -n "$untracked" ]]; then
    echo "sync-hextile: refuse — untracked files in $SUB (e.g. $untracked)" >&2
    exit 1
  fi
fi

echo "sync-hextile: marketplace root $ROOT"
echo "sync-hextile: fetching submodule remote tip…"
...
ver="$(read_version "$ROOT/$SUB/.claude-plugin/plugin.json")" || {
  echo "sync-hextile: refuse — cannot read plugin version" >&2
  exit 1
}

# No-op when HEAD gitlink already matches worktree tip
if [[ -n "$old_sha" && "$old_sha" == "$new_sha" ]]; then
  echo "sync-hextile: already up to date at $short_sha (v$ver)"
  exit 0
fi
...
msg="chore: pin hextile ${short_sha} (v${ver})"
...
echo "sync-hextile: committed pin $short_sha (v$ver)  [was $old_disp]"
echo "sync-hextile: $msg"

if [[ "$PUSH" -eq 1 ]]; then
  echo "sync-hextile: pushing…"
  git push origin HEAD
  echo "sync-hextile: push ok"
else
  echo "sync-hextile: pin local only (pass --push to push origin)"
fi

echo "sync-hextile: hosts still need marketplace update / reinstall hextile@360-hextile"
exit 0
